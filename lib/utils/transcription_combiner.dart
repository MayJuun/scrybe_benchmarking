...
class TranscriptionCombiner {
  ...
  String combineTranscripts(String existing, String newText) {
    ...
    print('cleanExisting: $cleanExisting');
    print('cleanNew: $cleanNew');
    ...
    final (overlapStart, overlapScore) =
    ...
    if (overlapScore >= config.similarityThreshold) {
      ...
      _updateState(cleanNew);
      print('Result (merged at overlap): $result');
      return result;
    }
    ...
    if (_isProgressiveUpdate(cleanExisting, cleanNew)) {
      _updateState(cleanNew);
      print('Result (progressive update): $cleanNew');
      return cleanNew;
    }

    // THIS IS THE CRITICAL PART: Instead of just returning cleanExisting,
    // we need to append the new text if it's different
    if (cleanExisting.toLowerCase() != cleanNew.toLowerCase()) {
      final combined = '$cleanExisting $cleanNew';
      _updateState(cleanNew);
      print('Result (appended): $combined');
      return combined;
    }

    _updateState(cleanNew);
    print('Result (unchanged): $cleanExisting');
  ...
  bool _isProgressiveUpdate(String existing, String newText) {
    ...
    for (var i = 0; i < minNgrams; i++) {
      if (_calculateSimilarity(existingNgrams[i], newStartNgrams[i]) >=
          config.similarityThreshold) {
        matchCount++;
      }
    }
